				...
				<div id="board-search">
					<div class="container">
						<div class="search-window">
							<form action="Community" class="formCss">
								<input type="hidden" name="table" value="${param.table }">
								<input type="hidden" name="page" value="${pageInfo.getPageNum() }">
								...
								<label for="search" class="blind">${msg } 내용 검색</label>
...
<%-- 											<c:if test="${msg eq 'QnA'}"><c:forEach var="i"begin="1" end="${list.getRe_lev() }">&nbsp;</c:forEach></c:if> --%>
											${list.getTitle() }</td>
				...
				<section id="pageList">
					<c:choose>
						<c:when test="${pageNum > 1}">
							<input type="button" value="이전"
								onclick="location.href='NoticeList?page=${pageNum - 1}'">
						</c:when>
						<c:otherwise>
							<input type="button" value="이전">
						</c:otherwise>
					...
					<c:choose>
						<c:when test="${pageNum < maxPage}">
							<input type="button" value="다음" onclick="location.href='NoticeList?page=${pageNum + 1}'">
						</c:when>
						<c:otherwise>
